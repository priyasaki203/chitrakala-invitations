...
async function insertEnquiry(form) {
  const { error } = await supabase.from("enquiries").insert([{
    name: form.name, phone: form.phone, email: form.email,
    service: form.service, event_date: form.event_date || null,
    message: form.message, submitted_at: new Date().toISOString(),
  }]);
  if (error) throw new Error(error.message);
}
...
async function fetchEnquiries() {
  ...
  if (error) { console.error("fetchEnquiries:", error.message); return []; }
  return data;
}
...
.contact-sec{background:linear-gradient(160deg,#fff7fb 0%,#fce8f3 50%,#fdf3dc 100%);padding:4rem 2rem;position:relative;overflow:hidden}
.contact-sec::before{content:'';position:absolute;top:-80px;right:-60px;width:320px;height:320px;border-radius:50%;background:radial-gradient(circle,rgba(232,24,109,0.06),transparent 70%);pointer-events:none}
.contact-wrap{max-width:1100px;margin:0 auto;display:grid;grid-template-columns:1fr 1.4fr;gap:3rem;align-items:start}
.contact-left h2{font-family:'Cormorant Garamond',serif;font-size:2.6rem;font-weight:700;color:var(--txt);line-height:1.15;margin-bottom:0.5rem}
.contact-left h2 em{color:var(--pk);font-style:italic;display:block}
...
.brochure-btn{display:inline-flex;align-items:center;gap:10px;background:linear-gradient(135deg,var(--gd),var(--gd2));color:#fff;border:none;border-radius:var(--r2);padding:13px 22px;font-size:0.88rem;font-weight:700;font-family:'DM Sans',sans-serif;cursor:pointer;transition:var(--tr);box-shadow:0 4px 16px rgba(201,149,42,0.3);text-decoration:none}
.brochure-btn:hover{box-shadow:0 8px 28px rgba(201,149,42,0.45);transform:translateY(-2px)}
.brochure-btn:disabled{opacity:0.55;cursor:not-allowed;transform:none}
.contact-form{background:#fff;border-radius:20px;padding:2rem;box-shadow:var(--sh2);border:1px solid rgba(232,24,109,0.08)}
.cf-grid{display:grid;grid-template-columns:1fr 1fr;gap:1rem;margin-bottom:1rem}
.cf-full{grid-column:1/-1}
.cf-field{display:flex;flex-direction:column;gap:5px}
.cf-label{font-size:0.72rem;font-weight:700;color:var(--txt2);text-transform:uppercase;letter-spacing:0.6px}
.cf-input{padding:11px 14px;border:1.5px solid rgba(232,24,109,0.16);border-radius:var(--r2);font-family:'DM Sans',sans-serif;font-size:0.9rem;color:var(--txt);outline:none;transition:border-color 0.2s;background:#fff;width:100%}
.cf-input:focus{border-color:var(--pk);box-shadow:0 0 0 3px rgba(232,24,109,0.07)}
.cf-select{padding:11px 14px;border:1.5px solid rgba(232,24,109,0.16);border-radius:var(--r2);font-family:'DM Sans',sans-serif;font-size:0.9rem;color:var(--txt);outline:none;background:#fff;cursor:pointer;width:100%}
.cf-textarea{padding:11px 14px;border:1.5px solid rgba(232,24,109,0.16);border-radius:var(--r2);font-family:'DM Sans',sans-serif;font-size:0.9rem;color:var(--txt);outline:none;transition:border-color 0.2s;background:#fff;width:100%;resize:vertical;min-height:110px}
.cf-textarea:focus{border-color:var(--pk);box-shadow:0 0 0 3px rgba(232,24,109,0.07)}
.cf-submit{width:100%;padding:14px;border:none;border-radius:var(--r2);background:linear-gradient(135deg,var(--pk),var(--pk2));color:#fff;font-size:0.95rem;font-weight:700;font-family:'DM Sans',sans-serif;cursor:pointer;transition:var(--tr);margin-top:6px;box-shadow:0 4px 18px rgba(232,24,109,0.3);letter-spacing:0.3px}
.cf-submit:hover:not(:disabled){box-shadow:0 8px 30px rgba(232,24,109,0.45);transform:translateY(-1px)}
.cf-submit:disabled{opacity:0.55;cursor:not-allowed;transform:none}
.cf-success{text-align:center;padding:2rem;display:flex;flex-direction:column;align-items:center;gap:10px}
.cf-success-icon{font-size:3rem;animation:pulse 2s infinite}
.cf-success h3{font-family:'Cormorant Garamond',serif;font-size:1.6rem;font-weight:700;color:var(--txt)}
.cf-success p{font-size:0.88rem;color:var(--txt3)}
.enq-row{padding:14px 16px;border-bottom:1px solid var(--pk4);display:flex;flex-direction:column;gap:4px}
.enq-row:last-child{border-bottom:none}
.enq-row:hover{background:#fff9fc}
.enq-name{font-size:0.9rem;font-weight:700;color:var(--txt)}
.enq-meta{font-size:0.75rem;color:var(--txt3);display:flex;gap:14px;flex-wrap:wrap}
.enq-msg{font-size:0.82rem;color:var(--txt2);margin-top:3px;line-height:1.5}
.enq-svc{background:var(--pk4);color:var(--pk2);padding:2px 10px;border-radius:var(--r3);font-size:0.7rem;font-weight:700;display:inline-block}
@media(max-width:768px){.contact-wrap{grid-template-columns:1fr}.cf-grid{grid-template-columns:1fr}}
...
<title>Chitrakala Invitations — Brochure</title>
<style>
  @import url('https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,400;0,600;0,700;1,400;1,600&family=DM+Sans:wght@300;400;500;600&display=swap');
  *{box-sizing:border-box;margin:0;padding:0}
  body{font-family:'DM Sans',sans-serif;background:#fff;color:#1a0a12;max-width:800px;margin:0 auto;padding:40px 32px}
  h1{font-family:'Cormorant Garamond',serif;font-size:3rem;font-weight:700;color:#1a0a12;text-align:center;margin-bottom:4px}
  h1 span{color:#e8186d;font-style:italic}
  .tagline{text-align:center;font-size:1rem;color:#9a6878;margin-bottom:8px}
  .gold-bar{width:60px;height:3px;background:linear-gradient(to right,#c9952a,#e8b84b);border-radius:4px;margin:12px auto 32px}
  .hero-blurb{text-align:center;font-size:1.05rem;color:#5c2d45;line-height:1.7;max-width:560px;margin:0 auto 36px}
  h2{font-family:'Cormorant Garamond',serif;font-size:1.6rem;font-weight:700;color:#1a0a12;margin-bottom:16px;border-left:4px solid #e8186d;padding-left:12px}
  .cat-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-bottom:36px}
  .cat-card{border:1.5px solid #f9c6dd;border-radius:14px;padding:20px 16px;text-align:center}
  .cat-icon{font-size:2rem;margin-bottom:8px}
  .cat-name{font-family:'Cormorant Garamond',serif;font-size:1.15rem;font-weight:700;color:#1a0a12;margin-bottom:4px}
  .cat-desc{font-size:0.8rem;color:#9a6878;line-height:1.5}
  .pricing{background:linear-gradient(135deg,#fff7fb,#fdf3dc);border-radius:14px;padding:24px;margin-bottom:36px}
  .price-row{display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:1px solid #f9c6dd}
  .price-row:last-child{border-bottom:none}
  .price-name{font-size:0.92rem;color:#1a0a12;font-weight:500}
  .price-val{font-size:1rem;font-weight:700;color:#e8186d}
  .features{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:36px}
  .feat{display:flex;align-items:flex-start;gap:10px;font-size:0.88rem;color:#5c2d45;line-height:1.5}
  .feat-dot{color:#e8186d;font-size:1.1rem;margin-top:-1px;flex-shrink:0}
  .contact-box{background:#1a0a12;border-radius:14px;padding:28px;color:#fff;text-align:center}
  .contact-box h2{color:#f9c6dd;border-color:#e8186d;font-size:1.4rem}
  .contact-box p{font-size:0.9rem;color:rgba(255,255,255,0.65);margin:8px 0}
  .contact-box a{color:#e8b84b;text-decoration:none}
  .footer-note{text-align:center;font-size:0.75rem;color:#9a6878;margin-top:28px}
  @media print{body{padding:20px}}
</style>
</head>
<body>
  <h1>Chitrakala <span>Invitations</span></h1>
  <div class="tagline">Premium Digital Invitations for Every Celebration</div>
  <div class="gold-bar"></div>
  <p class="hero-blurb">We craft stunning digital invitations that capture the essence of your special day. Handcrafted designs, quick delivery, and fully personalised to match your vision.</p>

  <h2>Our Categories</h2>
  <div class="cat-grid">
    <div class="cat-card">
      <div class="cat-icon">💍</div>
      <div class="cat-name">Wedding</div>
      <div class="cat-desc">Traditional & modern designs for every wedding theme — save the dates, main invites & more.</div>
    </div>
    <div class="cat-card">
      <div class="cat-icon">🏡</div>
      <div class="cat-name">Housewarming</div>
      <div class="cat-desc">Beautiful invitations to welcome family & friends to your new home celebration.</div>
    </div>
    <div class="cat-card">
      <div class="cat-icon">🎂</div>
      <div class="cat-name">Birthday</div>
      <div class="cat-desc">Fun, elegant, or themed — we design birthday invites for all ages and occasions.</div>
    </div>
  </div>

  <h2>Pricing (Starting From)</h2>
  <div class="pricing">
    <div class="price-row"><span class="price-name">Static Image Invitation</span><span class="price-val">₹299</span></div>
    <div class="price-row"><span class="price-name">Animated Video Invitation</span><span class="price-val">₹599</span></div>
    <div class="price-row"><span class="price-name">Premium Custom Design</span><span class="price-val">₹999+</span></div>
    <div class="price-row"><span class="price-name">Full Wedding Package</span><span class="price-val">₹1,999+</span></div>
  </div>

  <h2>What You Get</h2>
  <div class="features">
    <div class="feat"><span class="feat-dot">✦</span>High-resolution digital files (JPG/MP4)</div>
    <div class="feat"><span class="feat-dot">✦</span>Fully customised with your details</div>
    <div class="feat"><span class="feat-dot">✦</span>WhatsApp & social media ready</div>
    <div class="feat"><span class="feat-dot">✦</span>Delivered within 24–48 hours</div>
    <div class="feat"><span class="feat-dot">✦</span>Unlimited revisions until satisfied</div>
    <div class="feat"><span class="feat-dot">✦</span>All major Indian languages supported</div>
    <div class="feat"><span class="feat-dot">✦</span>Video invites with music & animation</div>
    <div class="feat"><span class="feat-dot">✦</span>Dedicated support via WhatsApp</div>
  </div>

  <div class="contact-box">
    <h2>Get In Touch</h2>
    <p>📧 <a href="mailto:priyasaki190@gmail.com">priyasaki190@gmail.com</a></p>
    <p>📱 <a href="https://wa.me/919840903746">+91 98409 03746 (WhatsApp)</a></p>
    <p>📸 <a href="https://instagram.com/priyasaki__19">@priyasaki__19</a></p>
    <p style="margin-top:14px;font-size:0.8rem;color:rgba(255,255,255,0.4)">chitrakala-invitations.vercel.app</p>
  </div>
  <div class="footer-note">© 2025 Chitrakala Invitations · All rights reserved</div>
</body>
</html>`;
...
function ContactSection({ toast }) {
  const blank = { name: "", phone: "", email: "", service: "", event_date: "", message: "" };
    ...
    <section className="contact-sec" id="contact">
      <div className="contact-wrap">
        ...
        <div className="contact-left">
          ...
          <button className="brochure-btn" onClick={() => downloadBrochure(toast)}>
            📄 Download Brochure
          </button>
        </div>
...
function AdminDash({ templates, onAdd, onEdit, onDelete, onToggle, onLogout }) {
  ...
  useEffect(() => { if (tab === "enquiries") fetchEnquiries().then(setEnquiries); }, [tab]);
    ...
    <div className="adm">
      ...
      <div className="stats-row">
        {[["Total", total, "var(--pk)"], ["Active", active, "#27ae60"], ["Hidden", total - active, "var(--pk)"],
          ...
          ["Enquiries", enquiries.length, "#7b1fa2"]
        ].map(([label, val, color]) => (
      ...
      <div className="adm-tabs">
        ...
        <button className={`adm-tab${tab === "enquiries" ? " on" : ""}`} onClick={() => setTab("enquiries")}>📩 Enquiries ({enquiries.length})</button>
      </div>
      ...
      {tab === "enquiries" && (
        <div className="tbl-wrap">
          <div style={{ padding: "13px 16px", borderBottom: "1px solid var(--pk4)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <span style={{ fontSize: "0.85rem", fontWeight: 700, color: "var(--txt2)" }}>Customer Enquiries</span>
            <span className="ucnt">{enquiries.length} total</span>
          </div>
          {enquiries.length === 0 && <div style={{ padding: "2.5rem", textAlign: "center", color: "var(--txt3)" }}>No enquiries yet.</div>}
          {enquiries.map((e, i) => (
            <div key={i} className="enq-row">
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
                <div className="enq-name">{e.name}</div>
                {e.service && <span className="enq-svc">{e.service}</span>}
              </div>
              <div className="enq-meta">
                {e.phone  && <span>📱 {e.phone}</span>}
                {e.email  && <span>✉️ {e.email}</span>}
                {e.event_date && <span>📅 {e.event_date}</span>}
                <span style={{ marginLeft: "auto" }}>{new Date(e.submitted_at).toLocaleString("en-IN")}</span>
              </div>
...
function HomePage({ templates, session: sess, isAdmin, onEdit, onDelete, onToggle, onLoginClick, toast }) {
  ...
  return (
    <>
      ...
      {/* Contact / Enquiry section — shown to all logged-in users */}
      {sess && <ContactSection toast={toast} />}
    </>
  );
}
