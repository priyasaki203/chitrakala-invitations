      ...
      <div className="tcard-img-wrap">
        ...
        {isAdmin && (
          <div className="adm-ctrl">
            <button className="acb" onClick={() => onToggle(tpl.id)} title={tpl.is_active ? "Deactivate" : "Activate"}>{tpl.is_active ? "👁️" : "🙈"}</button>
  ...
  const submit = () => {
    ...
    if (isAdminUser) {
      ...
      setTimeout(() => {
        ...
        saveSession(s); onLogin(s); setLoading(false);
      }, 400);
    } else {
  ...
  return (
    <div className="overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal wide">
        ...
        {err && <div className="errmsg">⚠️ {err}</div>}
        <button className="sub-btn" onClick={save}>{tpl ? "Save Changes" : "Add Template"}</button>
      </div>
    </div>
  );
}
    ...
    <div className="adm">
            ...
            <tbody>
              ...
              {templates.map(t => (
                <tr key={t.id}>
                  ...
                  <td><button className="tog" onClick={() => onToggle(t.id)}>{t.is_active ? "✅" : "❌"}</button></td>
                  <td>
      ...
      {tab === "users" && (
        <div className="tbl-wrap">
          ...
          {users.length === 0 && <div style={{ padding: "2.5rem", textAlign: "center", color: "var(--txt3)" }}>No users signed in yet.</div>}
          {users.map((u, i) => (
    ...
    <>
      ...
      {editTpl && (
        <TplForm tpl={editTpl}
          ...
          onSave={data => { onEdit(data); setEditTpl(null); toast("Template updated! ✅"); }}
        />
      )}
    </>
  );
...
export default function App() {
  ...
  const [page, setPage] = useState(() => getSession()?.role === "admin" ? "admin" : "home");
  ...
  const handleLogin = (s) => {
    setSession(s);
    setShowLogin(false);
    if (s.role === "admin") { setPage("admin"); toast("Welcome back, Admin! 🛡️"); }
  ...
  const delT = id => {
    ...
    setTemplates(p => p.filter(x => x.id !== id)); toast("Template deleted.");
  };
  const togT = id => {
    setTemplates(p => p.map(x => x.id === id ? { ...x, is_active: !x.is_active } : x));
  ...
  return (
    <>
      ...
      <header className="hdr">
        <div className="hdr-in">
          ...
          <nav className="nav-right">
            {session ? (
              <>
                <span className="chip-user">{isAdmin ? "🛡️ Admin" : `👤 ${session.name}`}</span>
                {isAdmin && (
      ...
      {page === "admin" && isAdmin
        ...
        : <HomePage templates={templates} session={session} isAdmin={isAdmin}
